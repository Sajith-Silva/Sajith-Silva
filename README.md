# 👋 Hi, I'm Sajith Silva

**Aspiring Cybersecurity Professional** breaking into the industry — passionate about threat detection, incident response, and building a safer digital world.

🟢 Open to entry-level SOC Analyst or IAM Security roles  
📍 Port Macquarie, NSW, Australia

---

## 🎯 About me

I'm an IT professional currently working as a Network Support Analyst while actively transitioning into a dedicated cybersecurity role. In my current position I manage the full Microsoft security stack — including Defender for Endpoint, Identity, Cloud Apps, and Office 365 — and have driven measurable outcomes including a 50% improvement in threat detection rates and a 40% reduction in high-risk endpoint vulnerabilities.

My day-to-day involves administering Microsoft Intune for device lifecycle management, designing Conditional Access policies in Entra ID, and conducting root cause analysis on suspicious identity and M365 login events. I also lead vulnerability assessments using Nessus, Wireshark, and Nmap, and manage patch cycles for Windows environments.

Beyond the tools, I've implemented and maintained ISO 27001-aligned security controls that improved network resilience by 30%, and I've contributed to ICT security uplift projects across cloud and on-premises infrastructure. I thrive in cross-functional teams and enjoy bridging the gap between security, infrastructure, and service delivery.

---

## 🎓 Certifications

![CompTIA Security+](https://img.shields.io/badge/CompTIA-Security%2B-red?style=flat-square&logo=comptia)
![ISO 27001](https://img.shields.io/badge/ISO-27001%20Lead%20Auditor-blue?style=flat-square)


---

## 🛠️ Skills & tools

![Microsoft Defender](https://img.shields.io/badge/Microsoft-Defender-0078D4?style=flat-square&logo=microsoft)
![Entra ID](https://img.shields.io/badge/Microsoft-Entra%20ID-0078D4?style=flat-square&logo=microsoftazure)
![SIEM](https://img.shields.io/badge/SIEM-Monitoring-555?style=flat-square)
![Incident Response](https://img.shields.io/badge/Incident-Response-orange?style=flat-square)
![Vulnerability Management](https://img.shields.io/badge/Vulnerability-Management-purple?style=flat-square)
![Endpoint Security](https://img.shields.io/badge/Endpoint-Security-darkgreen?style=flat-square)
![ISO 27001](https://img.shields.io/badge/ISO-27001-blue?style=flat-square)

---

## 📌 What I'm working on

- 🏠 **Home lab** — Setting up a personal SOC environment to practise threat detection and log analysis
- 📚 **Continuous learning** — Studying for further certifications and following the latest in threat intelligence
- 🚩 **CTF challenges** — Participating in Capture the Flag competitions to sharpen practical skills
- 🤝 **Community** — Connecting with the security community and contributing to open discussions

---

## 📫 Get in touch

I'm always happy to connect with fellow security enthusiasts, mentors, or hiring managers!

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-blue?style=flat-square&logo=linkedin)](https://www.linkedin.com/in/sajith-gajasinghege)

---

*"Security is not a product, but a process." — Bruce Schneier*
